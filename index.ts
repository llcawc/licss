  ...
  return through2.obj(async function (file: File, _, cb) {
    ...
    if (file.isBuffer()) {
      try {
        ...
        cb(new PluginError('licss', err as string | Error, { fileName: file.path }))
      }
    }
    cb(null, file)
  })
...
function mess(file: File, message: string) {
...
async function handleLightningCSSCompilation(
  file: File,
  minify: boolean,
  prefix: boolean,
  targetsList: Targets,
  ...
  if (file.isBuffer()) {
    ...
    if (result.map) file.sourceMap = JSON.parse(new TextDecoder().decode(result.map))
  }
}

// Bundle CSS with LightningCSS
async function handleLightningcssBundle(
  file: File,
  minify: boolean,
  prefix: boolean,
  targetsList: Targets,
...
// Compile sass/scss
async function handleSassCompilation(file: File, minify: boolean, loadPaths: string[], makeSourceMap: boolean) {
  if (file.isBuffer()) {
...
function addSourceMap(file: File, map: RawSourceMap) {
  if (file.isBuffer()) {
...
// Purge unused CSS
async function purgeTransform(file: File, options: UserDefinedOptions) {
  if (file.isBuffer()) {
    const processedContent = getFiles(options.content as string[], options.skippedContentGlobs)
