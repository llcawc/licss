...
vi.mock('lightningcss', () => ({
  ...
  transform: vi.fn(() => ({
    code: new Uint8Array([99, 115, 115]),
    map: null,
  })),
}))
...
describe('licss', () => {
  ...
  it('должен обрабатывать CSS файл с компилятором lightningcss по умолчанию', async () => {
    ...
    // Содержимое должно быть заменено на мок
  ...
  it('должен обрабатывать SCSS файл с компилятором sass', async () => {
    const plugin = licss({ compiler: 'sass', minify: false })
    const mockFile = createMockFile({
      path: '/test/file.scss',
  ...
  it('должен минифицировать CSS при minify: true', async () => {
    ...
    await plugin._transform(mockFile, null, callback)
    expect(callback).toHaveBeenCalledWith(null, mockFile)
    // LightningCSS мок возвращает 'css', минификация не влияет
    expect((mockFile.contents as Buffer).toString()).toBe('css')
  })
  ...
  it('должен вызывать ошибку при неверном компиляторе', async () => {
    const plugin = licss({ compiler: 'invalid' as any })
    const mockFile = createMockFile({
      path: '/test/file.css',
      contents: Buffer.from('body { color: red; }'),
    })
    const callback = vi.fn()
    // @ts-expect-error доступ к приватному методу
    await expect(plugin._transform(mockFile, null, callback)).rejects.toThrow('Unsupported "compiler" option')
    // Колбек не должен быть вызван
    expect(callback).not.toHaveBeenCalled()
  })
})
